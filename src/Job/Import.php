...
class Import extends AbstractJob
{
    ...
    public function perform()
    {
        ...
        if(!isset($this->actant[$this->zAutoTag])){
            $this->actant[$this->zAutoTag] = $this->ajouteActant($this->zAutoTag, $this->zAutoTag);        
        }
        ...
        foreach (array_chunk($zItemKeys, 50, true) as $zItemKeysChunk) {
            ...
            foreach ($zItems as $zItem) {
                $dateVerif = new DateTime($zItem['data']['dateAdded']);
                $dateVerif = new DateTime($zItem['data']['dateAdded']);                
                if ($dateVerif->getTimestamp() < $this->getArg('timestamp', 0)) {
                    // Only import items added since the passed timestamp. Note
                    // that the timezone must be UTC.
                    $this->logger->info("date ajout = ".$dateVerif->getTimestamp()." < date après ".$this->getArg('timestamp'));
                    continue;
                }
                if ($dateVerif->getTimestamp() > $this->getArg('timestampBefore')) {
                    // Only import items added before the passed timestamp. Note
                    // that the timezone must be UTC.
                    $this->logger->info("date ajout = ".$dateVerif->getTimestamp()." > date avant ".$this->getArg('timestampBefore'));
                    continue;
                }
                ...
                $faitPar= $zItem['meta']['createdByUser'] ? $zItem['meta']['createdByUser'] : $this->actant[$this->url->id()]->displayTitle();
        ...
        foreach ($zParentItems as $zParentItemKey => &$zParentItem) {
            ...
            $faitPar = $this->actant[$this->url->id()]->displayTitle();
            if(isset($zParentItem['meta']['createdByUser'])){
                if(!isset($this->actant[$zParentItem['meta']['createdByUser']['username']])){
                    $this->actant[$zParentItem['meta']['createdByUser']['username']] = $this->ajouteActant($zParentItem['meta']['createdByUser']['id'], $zParentItem['meta']['createdByUser']['username']);        
                }
                $faitPar=$this->actant[$zParentItem['meta']['createdByUser']['username']]->displayTitle()."";
            }


            //récupération des tags pour différencier 
            //tags de l'utilisateur et ceux créés automatiquement
            $urlTag = $this->url->itemTags($zParentItem['key'],[
                'key' => $apiKey,
            ]);
            $this->logger->info('urlTag='.$urlTag);
            ...
            foreach ($zTags as $t) {
                ...
                $this->tags[$keyTag]['actants'][$actant][]=$zParentItem['key'];
            }
            ...
            if($this->url->type()=='groups'){
                //récupération de l'item pour avoir le addedBy
                $urlItemGroup = $this->url->item($zParentItem['key'],[
                    'key' => $apiKey,
                ]);
                $this->logger->info('urlItemGroup='.$urlItemGroup);
                $zItemGroup = json_decode($this->getResponse($urlItemGroup)->getBody(),true);
                if(!isset($this->actant[$zItemGroup['meta']['createdByUser']['username']])){
                    $this->actant[$zItemGroup['meta']['createdByUser']['username']] = $this->ajouteActant($zItemGroup['meta']['createdByUser']['id'], $zItemGroup['meta']['createdByUser']['username']);        
                }
                $valueObject=[];
                $valueObject['property_id'] = $this->properties["cito"]["isCompiledBy"]->id();
        ...
        /*batch abandonné car problème de lien avec la suite du traitement
        foreach (array_chunk($oItems, 50, true) as $oItemsChunk) {
        */
        foreach ($oItems as $zKey=>$oItemsChunk) {


            if ($this->shouldStop()) {
                return;
            }
            $response = $this->api->create('items', $oItemsChunk, [], ['continueOnError' => true]);
            $item = $response->getContent();
            $this->logger->info('item crée : '.$zKey.' : '.$item->id().' : '.$item->displayTitle());
            ...
            $this->api->create('zotero_importplus_items', $importItems, [], ['continueOnError' => true]);
        }
        ...
        foreach ($this->tags as $tag) {
            ...
            foreach ($tag['actants'] as $act => $vals) {
                $this->logger->info('act = '.$act.' vals = '.json_encode($vals));
                if(!$this->actant[$act]){
                ...
                foreach ($vals as $zKey) {
                    foreach ($oTags as $oTag) {
                        //if($oItems[$zKey])
                        $this->ajouteAnnotation($oItems[$zKey],$this->actant[$act], $oTag);                        
                    }                                        
                }
            }
        }        
    ...
    {
        $this->logger->info("ajouteActant ".$id.' '.$username);            
        ...
        if(count($result)){
            ...
            $this->logger->info("actant ajouté",$oItem);            
    ...
    {
        $this->logger->info("ajouteTag ",$tag);            
        //prise en compte des -
